...
def line_detection(grid):
	...
	y_counter={}
	firsttime=0
	#possible_orientation=0 means its horizontal line and possible_orientation=1 then its vertical line
...
def calculate_slopes(possible_orientation,x_counter,y_counter,black_pixels):
	for key in x_counter:
		print(key)
		for coordinates in black_pixels:
			x=coordinates[0]
			y=coordinates[1]
			if x==key:
				print("no we are not doing this")
def grid_partition():
	...
	#calculate_slopes(possible_orientation,x_counter,y_counter,black_pixels)
