...
def create_image(contineous_region):
	...
	# plt.axis([0, 20, 0, 20])
	# plt.show()
def modify_created_image(contineous_region,gray):
	new_gray = gray.copy()
	new_contineous=contineous_region.copy()
	for coordinates in new_contineous:
		new_gray[coordinates[1],coordinates[0]]=(255)
	cv2.imwrite("temp.png",new_gray)
...
def main(filename):
	...
	contineous_region_length=len(contineous_region)
	modify_created_image(contineous_region,gray)
	for idx,data in enumerate(contineous_region):
		if idx==0:
			data1 = data
		if idx==contineous_region_length-1:
			character_width = data[0]-data1[0]
			return data1,character_width
